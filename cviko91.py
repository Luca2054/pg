...
def filtruj_cisla(typ, cisla):
    ...
    # Vaše řešení zde
    if typ == "kladna":
        for cislo in cisla:#forcyklus - vezme sekvenci (-> seznam čísel)z cisla polozka po polozce a zkontroluje jestli je kladná
            if cislo > 0:
        ...
        for cislo in cisla:
            if cislo % 2 == 0: #zbytek po dělení
                vysledek.append(cislo)
    elif typ == "licha":
        for cislo in cisla:
            if cislo % 2 != 0:#není roven nule
                vysledek.append(cislo)
    else:
        vysledek = []
    return vysledek
