...
def spocitej_statistiku(text):
    ...
    pocet_radku = len(text.split('\n'))# nový řádek (text procházíme několikrát)
    pocet_slov = len(text.split())
    pocet_znaku = len(text)
#vyjímka se vyvolává pomocí příkazu raise - pak hledá try except blok, který vyjímku ošetřuje

    # Vaše řešení zde
    #efektivní řešení projdeme znak po znaku  -> jen jednou 
